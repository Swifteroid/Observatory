...
extension ShortcutRecorder {
    ...
    public static var shortcutWillChangeNotification: Notification.Name { Notification.Name("\(ShortcutRecorder.self)HotkeyWillChangeNotification") }

    /// Posted after `ShortcutRecorder`'s `shortcut` value gets changed, this gets posted, both, when the shortcut is changed
    /// by outside code and when the new hotkey gets recorded.
    public static var shortcutDidChangeNotification: Notification.Name { Notification.Name("\(ShortcutRecorder.self)HotkeyDidChangeNotification") }
